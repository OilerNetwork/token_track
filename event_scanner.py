# code from: https://web3py.readthedocs.io/en/v5/examples.html#advanced-example-fetching-all-token-transfer-events
...
class EventScanner:
    ...
    # TKS: making these consts so that we can use them as default params
    MIN_SCAN_CHUNK_SIZE = 10 # 12 s/block = 120 seconds period

    def __init__(self, web3: Web3, contract: Contract, state: EventScannerState, events: List, filters: {},
                 max_chunk_scan_size: int = 100000, max_request_retries: int = 30, request_retry_seconds: float = 3.0):
        """
    ...
    def incr_call_count(self):
        if(self.call_count > 10000):
          raise Exception("Too many calls for now")
        self.call_count += 1
    ...
    def scan_chunk(self, start_block, end_block) -> Tuple[int, datetime.datetime, list]:
        ...
        for event_type in self.events:
            ...
            for evt in events:
                ...
                # TKS: I see no reason for timestamp requests, we can just save block numbers
                # block_when = get_block_when(block_number)
                # TKS: also fixing the logger since it had a wrong number of args
                # logger.debug("Processing event %s, block:%d count:%d", evt["event"], evt["blockNumber"])
                logger.debug("Processing event %s, block:%d count:%d", evt["event"], evt["blockNumber"], len(all_processed))
                # TKS: I see no reason for timestamp requests, we can just save block numbers
                #processed = self.state.process_event(block_when, evt)
                processed = self.state.process_event(block_number, evt)
    ...
    def estimate_next_chunk_size(self, current_chuck_size: int, event_found_count: int):
        ...
        * Do not overload node serving JSON-RPC API by asking data for too many events at a time
        ...
        """
        # TKS: introducing the decrease logic
        #if event_found_count > 0:
            # When we encounter first events, reset the chunk size window
            # TKS: wait - it means that it will never go above MIN if fetching any events? waaaaaait
            # current_chuck_size = EventScanner.MIN_SCAN_CHUNK_SIZE
        if (event_found_count > current_chuck_size or event_found_count > 1024): # TKS: my intuitive logic - more than one per block or more than 1024 decreases
            current_chuck_size *= self.chunk_size_decrease # TKS: decrease was not used before, probably was replaced to MIN after some errors as a quick fix
        else:
            current_chuck_size *= self.chunk_size_increase

        current_chuck_size = max(EventScanner.MIN_SCAN_CHUNK_SIZE, current_chuck_size)
        current_chuck_size = min(self.max_scan_chunk_size, current_chuck_size)
        logger.info(f"Chunk size set to {current_chuck_size}")
        return int(current_chuck_size)

    def scan(self, start_block, end_block, start_chunk_size=MIN_SCAN_CHUNK_SIZE, progress_callback=Optional[Callable]) -> Tuple[
        list, int]:
        ...
        while current_block <= end_block:
            ...
            # TKS: first use the chunk_size from failedd retries
            # TKS: ugly code follows - generally find the pev power of 2, not next :)
            if(estimated_end_block - actual_end_block > 0):
                chunk_size = max(_next_power_of_2_((chunk_size - (estimated_end_block - actual_end_block)) / 10) * 10 / 2, EventScanner.MIN_SCAN_CHUNK_SIZE)
...
if __name__ == "__main__":
    # TKS: why 20k calls for 11k transfers?
    # TKS: better call it carefully
    ...
    # OIL has around ? Transfer events
    # https://etherscan.io/token/0x0275E1001e293C46CFe158B3702AADe0B99f88a5
    OIL_ADDRESS = "0x0275E1001e293C46CFe158B3702AADe0B99f88a5"
    OIL_DEPLOYMENT_BLOCK = 12366916 # https://etherscan.io/tx/0x65020e85b05b8ee35804b1f593c54a84b060b824f6d7e77241122ef0e10e4ddb
    ...
    class JSONifiedState(EventScannerState):
        ...
        # TKS: decreasing number of calls by not using timestamps
        # def process_event(self, block_when: datetime.datetime, event: AttributeDict) -> str:
        def process_event(self, block_when: int, event: AttributeDict) -> str:
    ...
    def run():
        ...
        provider = HTTPProvider(ethereum.api_url)
        ...
        # chain_id: int, web3: Web3, abi: dict, state: EventScannerState, events: List, filters: {}, max_chunk_scan_size: int=100000
        scanner = EventScanner(
            web3=web3,
            contract=ERC20,
            state=state,
            events=[ERC20.events.Transfer],
            filters={"address": OIL_ADDRESS},
            # How many maximum blocks at the time we request from JSON-RPC
            # and we are unlikely to exceed the response size limit of the JSON-RPC server
            # TKS: changed this value to 100,000 as I do not expect much transfer density - still would like to make sure that the code does not crash on limit exceed
            # TKS: and gracefully goes to a smaller chunk? will see, would happily make it 1,000,000 here
            # TKS: ok, have to admit it is always failing at 10240... back to 5_120
            max_chunk_scan_size=5120
        )
        ...
        chain_reorg_safety_blocks = 128 # TKS: changed the reorg safety to something more in line with clients
        scanner.delete_potentially_forked_block_data(state.get_last_scanned_block() - chain_reorg_safety_blocks)

        # Scan from [last block scanned] - [latest ethereum block]
        # Note that our chain reorg safety blocks cannot go negative

        # TKS: changing start block to when the contract was deployed
        # TKS: feels like the code would ideally create a binary scan first to find the first non-empty response
        # TKS: not that in the original form the code would go in 10k chunk over 10M+ empty blocks so wasted 1k API calls
        start_block = max(OIL_DEPLOYMENT_BLOCK - 1, max(state.get_last_scanned_block() - chain_reorg_safety_blocks, 0))
